...
def Fun_diffraction(phi, matrixband, matrixPhaseBandGap, wavelennum, samplenum, Dx, T, N, bandnum, wavelen0, lam):
    ...
    ratio1, distance1, strength1, efficiency, ItotalDisplay_sum, ItotalDisplay_incident_sum = AngularSpectrumDiffraction(Ex0, Ey0, wavelennum, samplenum, Dx, wavelen0, lam)
...
def AngularSpectrumDiffraction(Ex0, Ey0, wavelennum, samplenum, Dx, wavelen0, lam):
    ...
    Num = 51
    if wavelennum == 28:
        Zd1 = np.arange(287, 297 + 0.2, 0.2)*wavelen0
    if wavelennum == 19:
        Zd1 = np.arange(310, 320 + 0.2, 0.2)*wavelen0
    if wavelennum == 10:
        Zd1 = np.arange(337, 347 + 0.2, 0.2)*wavelen0
    if wavelennum == 1:
        Zd1 = np.arange(360, 370 + 0.2, 0.2) * wavelen0
    if wavelennum == 37:
        Zd1 = np.arange(272, 282 + 0.2, 0.2) * wavelen0
