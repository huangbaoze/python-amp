...
def Fun_diffraction(phi, matrixband, matrixPhaseBandGap, wavelennum, samplenum, Dx, T, N, bandnum, wavelen0, lam, f):
    ...
    ratio1, distance1, strength1, efficiency, ItotalDisplay_sum, ItotalDisplay_incident_sum = AngularSpectrumDiffraction(Ex0, Ey0, wavelennum, samplenum, Dx, wavelen0, lam, f)
    return ratio1, distance1, strength1, efficiency, ItotalDisplay_sum, ItotalDisplay_incident_sum

def AngularSpectrumDiffraction(Ex0, Ey0, wavelennum, samplenum, Dx, wavelen0, lam, f):

    global Zd1
    n_refra = 1
    nn = 250
    Num = 201
    Zd1 = np.arange(int(f[wavelennum - 1]) - 50, int(f[wavelennum - 1]) + 50.5, 0.5) * wavelen0
    ...
    # df3.to_excel('ZItotalDisplay.xlsx', index=False, header=False)
    ratio1, distance1, strength1, efficiency, ItotalDisplay_sum, ItotalDisplay_incident_sum = computing_plot(Zd1, ZItotalDisplay, wavelennum, Num, nn, Dx, wavelen0, lam, ItotalDisplay_incident, ItotalDisplaymax)
    return ratio1, distance1, strength1, efficiency, ItotalDisplay_sum, ItotalDisplay_incident_sum
