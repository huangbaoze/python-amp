...
def Fun_diffraction(phi, matrixband, matrixPhaseBandGap, wavelennum, samplenum, Dx, T, N, bandnum, wavelen0, lam, filenum):
    ...
    ratio1, distance1, strength1, SL, efficiency = AngularSpectrumDiffraction(Ex0, Ey0, wavelennum, samplenum, Dx, wavelen0, lam, filenum)
    return ratio1, distance1, strength1, SL, efficiency

def AngularSpectrumDiffraction(Ex0, Ey0, wavelennum, samplenum, Dx, wavelen0, lam, filenum):
    ...
    Zd1 = np.arange(195, 460 + 0.5, 0.5) * wavelen0
    #Zd1 = np.arange(145, 510 + 0.5, 0.5) * wavelen0
    ...
    filename = 'D:\\zhaofen\\huangbaoze\\A_gdgddfold\\total_optical_field\\otherwavelengths_verification\\matching_fold_random_' + str(filenum) +'\\ZItotalDisplay\\' + 'ZItotalDisplay' + str(wavelennum) + '.xlsx'
    df3.to_excel(filename, index=False, header=False)
    ratio1, distance1, strength1, SL, efficiency = computing_plot(Zd1, ZItotalDisplay, wavelennum, Num, nn, Dx, wavelen0, lam, ItotalDisplay_incident, ItotalDisplaymax, filenum)
    return ratio1, distance1, strength1, SL, efficiency
...
def computing_plot(Z, AllDisplay, wavelennum, Num, nn, Dx, wavelen0, lam, ItotalDisplay_incident, ItotalDisplaymax, filenum):
    ...
    file4 = 'D:\\zhaofen\\huangbaoze\\A_gdgddfold\\total_optical_field\\otherwavelengths_verification\\matching_fold_random_' + str(filenum) + '\\ZItotalDisplay_SideLobe_Imax_SL_ItotalDisplay_all.xlsx'
    df4 = pd.DataFrame(data.get())
    if wavelennum == 1:
        df4.to_excel(file4, sheet_name='Sheet1', startrow=wavelennum - 1, header=False, index=False)
    else:
        book = load_workbook(file4)
        with pd.ExcelWriter(file4) as writer:
            writer.book = book
            writer.sheets = dict((ws.title, ws) for ws in book.worksheets)
            df4.to_excel(writer, sheet_name='Sheet1', startrow=wavelennum - 1, header=False, index=False)
    ...
    print(ItotalDisplay_sum)
    print(ItotalDisplay_incident_sum)
    print(efficiency)
    ...
    with pd.ExcelWriter(file4) as writer:
        writer.book = book
        writer.sheets = dict((ws.title, ws) for ws in book.worksheets)
        df5.to_excel(writer, sheet_name='Sheet1', startrow=wavelennum - 1, startcol=4, header=False, index=False)
