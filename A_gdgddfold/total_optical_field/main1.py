...
class PropagationPlot:
    def __init__(self):
        ...
        self.focallength = 295 * self.lamc #315
    ...
    def run(self, flag):
        ...
        global bandAall, bandPall, bandGDGDDall
        for i in range(self.N + 1):
            ...
            if i == 0:
                ...
                bandGDGDDall = np.zeros([int(sizeboun[i]), 2])
                bandGDGDDall[:] = bandGDGDD[:]
            else:
                ...
                bandGDGDDall1 = np.zeros([int(sizeboun[i]), 2])
                for l in range(2):
                    bandGDGDDall1[:, l] = np.append(bandGDGDDall[:, l], bandGDGDD[:, l])
                bandGDGDDall = bandGDGDDall1
        ...
        #0匹配排布
        if flag == 0:
            file = 'D:\\zhaofen\\huangbaoze\\A_gdgddfold\\A_higher_dimensional_DBS_copy\\data\\5_min_matrixband - 497.xlsx'
            ...
            for t in range(self.N + 1):
                ...
                matchinggap = np.abs(
                    matchingA[:, 20] - matrixband0[self.bandnum == t + 1].reshape(-1, 1))
        ...
        if flag == 1:
            ...
            for i in range(self.N + 1):
                if i == 0:
                    ...
                    # arraysum = np.random.randint(sizeboun[0], sizeboun[i])  #乱整检验
                for j in range(self.lamnum):
                    matrixband[j, self.bandnum == i + 1] = bandAall[arraysum, j]
                    matrixPhaseBandGap[j, self.bandnum == i + 1] = bandPall[arraysum, j]
                    # matrixPhaseBandGap[j, self.bandnum == i + 1] = bandGDGDDall[arraysum, 0] * (self.w[j] - self.w0) + bandGDGDDall[arraysum, 1] * (self.w[j] - self.w0) ** 2 #GD/GDD

        global filenum
        path = 'D:/zhaofen/huangbaoze/A_gdgddfold/total_optical_field/otherwavelengths_verification/matching_fold_random_' + str(
        ...
        fold_phase = np.array(pd.read_excel('D:\\zhaofen\\huangbaoze\\A_gdgddfold\\optimise_3lam\\data\\19_min_fold_phase.xlsx', header=None, sheet_name='Sheet1').values) #优化相位差
        lac_num = np.array(pd.read_excel('D:\\zhaofen\\huangbaoze\\A_gdgddfold\\optimise_3lam\\new_fold\\df_lac_num.xlsx', header=None, sheet_name=0).values)
        fold_phi = np.zeros(self.N + 1)
        fold_phi[:] = self.phi
        # fold_phase = np.random.rand(1, np.size(lac_num[0])) * 2 * np.pi #随机相位差
        for t in range(np.size(fold_phase[-1, ])):
            fold_phi[int(lac_num[0, t]) + 1:] = self.phi[int(lac_num[0, t]) + 1:] + fold_phase[-1, t]
        print(fold_phi)
        df_phi = pd.DataFrame(self.phi.get())
        df_phi.to_excel('D:\\zhaofen\\huangbaoze\\A_gdgddfold\\total_optical_field\\otherwavelengths_verification\\matching_fold_random_' + str(filenum) + '\\df_phi.xlsx',
                        sheet_name='Sheet1', startrow=0, header=False, index=False)
        df_fold_phi = pd.DataFrame(fold_phi.get())
        df_fold_phi.to_excel('D:\\zhaofen\\huangbaoze\\A_gdgddfold\\total_optical_field\\otherwavelengths_verification\\matching_fold_random_' + str(
                filenum) + '\\df_fold_phi.xlsx', sheet_name='Sheet1', startrow=0, header=False, index=False)

        # for i in range(self.lamnum):
        for i in (np.array([1, 11, 21, 31, 41]) - 1):
            i = int(i)
            ...
            file1 = 'D:\\zhaofen\\huangbaoze\\A_gdgddfold\\total_optical_field\\otherwavelengths_verification\\matching_fold_random_' + str(filenum) + '\\DL_FWHM_f_FL_Ipeak.xlsx'
        ...
        file2 = 'D:\\zhaofen\\huangbaoze\\A_gdgddfold\\total_optical_field\\otherwavelengths_verification\\matching_fold_random_' + str(filenum) + '\\min_matrixband.xlsx'
        df2 = pd.DataFrame(matrixband[20].get())
        df2.to_excel(file2, header=False, index=False)
        file3 = 'D:\\zhaofen\\huangbaoze\\A_gdgddfold\\total_optical_field\\otherwavelengths_verification\\matching_fold_random_' + str(filenum) + '\\matrixPhaseBandGap.xlsx'
        df3 = pd.DataFrame(matrixPhaseBandGap[20].get())
        df3.to_excel(file3, header=False, index=False)
