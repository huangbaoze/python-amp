...
def Fun_diffraction(phi, matrixband, matrixPhaseBandGap, wavelennum, samplenum, Dx, T, N, bandnum, wavelen0, lam):
    ...
    ratio1, distance1, strength1 = AngularSpectrumDiffraction(Ex0, Ey0, wavelennum, samplenum, Dx, wavelen0, lam)
    return ratio1, distance1, strength1
...
def AngularSpectrumDiffraction(Ex0, Ey0, wavelennum, samplenum, Dx, wavelen0, lam):
    ...
    Num = 51
    if wavelennum == 1:
        Zd1 = np.arange(357, 367 + 0.2, 0.2)*wavelen0
    if wavelennum == 28:
    ...
    ratio1, distance1, strength1 = computing_plot(Zd1, ZItotalDisplay, wavelennum, Num, nn, Dx, wavelen0, lam)
    return ratio1, distance1, strength1
...
def computing_plot(Z, AllDisplay, wavelennum, Num, nn, Dx, wavelen0, lam):
    ...
    b = (I2 - I1) / (x2 - x1)
    c = I2 - b * x2
    FWHM = (0.5 * strength1 - c) / b * 2
    ratio1 = FWHM / lam[wavelennum - 1]
